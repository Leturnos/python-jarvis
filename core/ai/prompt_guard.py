    ...
    def is_input_safe(cls, user_text: str) -> bool:
        ...
        for pattern in cls.SUSPICIOUS_PATTERNS:
            if re.search(pattern, user_text):
                logger.warning(
                    f"Prompt injection pattern detected in input: '{user_text}'"
                )
                return False
        return True
    ...
    def sanitize_output(cls, action_json: dict) -> dict:
        ...
        if "steps" in action_json:
            ...
            for step in action_json["steps"]:
                ...
                # Catastrophic blocks
                if any(
                    bad in cmd for bad in ["rm -rf /", "del /f /s /q c:\\", "format c:"]
                ) or any(bad in target for bad in ["windows", "system32"]):
                    logger.warning(
                        "Output sanitization: Blocked catastrophic step detected."
                    )
        ...
        action_json.get("risk_level", "safe")
        ...
        if action_type in ["system", "warp"]:
            ...
            for cmd in commands:
                ...
                if any(
                    bad in cmd_lower
                    for bad in ["rm -rf /", "del /f /s /q c:\\", "format c:"]
                ):
                    logger.warning(
                        f"Output sanitization: Blocked catastrophic command: {cmd}"
                    )
