...
from core.execution.job_queue import Job, JobStatus, JobType
...
def test_microphone_failure_and_healing(mock_reset):
    ...
    stream.read.side_effect = OSError("Device Unavailable")
    ...
    print(
        "[SUCCESS] The loop handled the microphone error and triggered the self-healing routine."
    )
...
def test_internet_down_retry(mock_completion, mock_cache_get):
    ...
    print(
        f"[SUCCESS] The worker retried {job.retries} times and failed gracefully with a network error."
    )
...
def test_gemini_rate_limit_429(mock_completion, mock_cache_get):
    ...
    mock_completion.side_effect = litellm.exceptions.RateLimitError(
        message="Rate limit exceeded", llm_provider="gemini", model="gemini-2.5-flash"
    )
    ...
    print(
        f"[SUCCESS] Rate Limit 429 error handled with complete retries ({job.retries}/2)."
    )
...
def test_database_logging_and_history(temp_env):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT metric_name, metric_value, tags FROM metrics ORDER BY id ASC"
    )
    ...
    db_mgr.close()
    print(
        "[SUCCESS] Commands saved, history retrieved, and metrics recorded asynchronously in SQLite."
    )
...
def test_llm_cache_operations(temp_env):
    ...
    print(
        "[SUCCESS] Normalization, hash storage, TTL expiration, and cache statistics verified."
    )
...
def test_light_stress_on_worker_queue():
    ...
    try:
        ...
        assert len(executed_jobs) == total_jobs, (
            "All 30 jobs should have been executed."
        )
        assert executed_jobs[0] == "cmd_0", (
            "The queue must process sequentially in FIFO order."
        )
        assert executed_jobs[-1] == f"cmd_{total_jobs - 1}"
        print(
            f"[SUCCESS] Queue sequentially processed {total_jobs} concurrent commands without deadlock."
        )
...
"""
    ...
    print(
        "[SUCCESS] YAML plugin read correctly, shared_actions expanded, and intent metadata verified."
    )
