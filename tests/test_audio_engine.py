...
from core.audio_engine import get_audio_stream, safe_reset_audio

@patch('core.audio_engine.pyaudio.PyAudio')
...
@patch('core.audio_engine.get_audio_stream')
def test_safe_reset_audio(mock_get_stream):
    ...
    with patch('core.audio_engine.time.sleep'):
...
def test_safe_reset_audio_handles_none(mock_pyaudio):
    ...
    with patch('core.audio_engine.time.sleep'):
        with patch('core.audio_engine.get_audio_stream', return_value=(MagicMock(), MagicMock())):
